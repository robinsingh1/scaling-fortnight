...
class OldCompanies:
    ...
    def _hiring(self, domain, api_key="", company_name="", daily=False):
        # paginate
        ...
        if daily: data = data[data.date.str.contains("hour|minute")]
    ...
    def _press_releases(self, domain, api_key="", company_name="", period=None):
        ''' Google News, PRNewsWire, BusinessWire '''
        if period:
          pw=Google().search('"{0}" site:prnewswire.com'.format(company_name),1,"d")
          bw=Google().search('"{0}" site:businesswire.com'.format(company_name),1,"d")
          mw=Google().search('"{0}" site:marketwired.com'.format(company_name),1,"d")
          nw=Google().search('"{0}" site:newswire.ca'.format(company_name),1,"d")
          rt=Google().search('"{0}" site:reuters.com'.format(company_name),1,"d")
        else:
    ...
    def _old_news(self, domain, api_key="", company_name=""):
    ...
    def _research_report(self, _report):
        ...
        for company in signals:
            ...
            q.enqueue(Companies()._google_search, company_name, api_key)
    ...
    def _google_search(self, name, api_key=""):
        ...
        for link in c.link:
          print link
          if "facebook" in link:
              q.enqueue(Facebook()._url_to_dict, name, link, api_key,timeout=6000)
          elif "twitter" in link:
              q.enqueue(Twitter()._url_to_dict, name, link, api_key,timeout=6000)
          elif "indeed" in link:
              q.enqueue(Indeed()._company_profile, name, api_key,timeout=6000)
          elif "businessweek" in link:
              q.enqueue(BusinessWeek()._company_profile, name, api_key,timeout=6000)
          elif "zoominfo" in link:
              q.enqueue(Zoominfo()._company_profile, name, api_key,timeout=6000)
          elif "linkedin" in link:
              q.enqueue(Linkedin()._company_profile, name, api_key,timeout=6000)
          elif "yellowpages" in link:
              q.enqueue(YellowPages()._company_profile, name, api_key,timeout=6000)
          elif "yelp" in link:
              q.enqueue(Yelp()._company_profile, name, api_key,timeout=6000)
          elif "forbes" in link:
              q.enqueue(Forbes()._company_profile, name, api_key,timeout=6000)
          elif "glassdoor" in link:
              q.enqueue(GlassDoor()._company_profile, name, api_key,timeout=6000)
          elif "hoovers" in link:
              q.enqueue(Hoovers()._company_profile, name, api_key,timeout=6000)
          elif "crunchbase" in link:
              q.enqueue(Crunchbase()._company_profile, name, api_key,timeout=6000)
        ...
        for i, row in d.iterrows():
            ...
            q.enqueue(Companies()._domain_search, row.domain, name, api_key, 
                      timeout=6000)
            if i > 5: break
    ...
    def _research(self, name, api_key="", prospect_name=""):
        ...
        j13= q.enqueue(Companies()._google_search, name, api_key,timeout=6000)
        j12= q.enqueue(ClearbitSearch()._company_profile, name, api_key,timeout=6000)
    ...
    def _daily_secondary_research(self, name, domain, api_key="", lists=[]):
        # Secondary Research - sometimes require location or domain
        if name == "": name = domain
        x = 600000
        ...
        jobs = [j0,j1,j2,j3,j4,j5,j6,j7,j8]
        for job in jobs: 
