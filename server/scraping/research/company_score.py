...
class CompanyScore:
    ...
    def _company_info(self, company_name, api_key="",domain_search=False):
        #TODO - company_name = self._remove_non_ascii(company_name) add to save
        if domain_search:
          qry = {'where':json.dumps({'domain': company_name}), 'limit':1000}
        else:
        ...
        if domain_search:
          ...
          self._add_to_clearspark_db('Company', 'domain', company_name, final)
        ...
        if RQueue()._has_completed("{0}_{1}".format(company_name, api_key)):
            ...
            #q.enqueue(Companies()._secondary_research, company_name, domain, api_key)
            if "domain" in final.keys():
                r = requests.get("http://floating-inlet-9798.herokuapp.com/v1/secondary_research", 
                    params={"company_name":final["company_name"], "domain":final["domain"]}) 
                print r.text
            else:
                ''' print "GOOGLE SEARCH" '''
            ...
            if "company_name" in final.keys():
                ...
                for domain in crawls.domain.dropna().drop_duplicates():
                    ...
                    r = requests.get("http://floating-inlet-9798.herokuapp.com/v1/email_pattern/research", 
